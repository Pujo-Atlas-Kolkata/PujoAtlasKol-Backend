...
from .serializers import (
    PujoSerializer,
    TrendingPujoSerializer,
    SearchedPujoSerializer,
    searchPujoSerializer,
)
...
class PujoViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            response_data = {"error": str(e), "status": ResponseStatus.FAIL.value}
    ...
    def retrieve(self, request, uuid=None, *args, **kwargs):
        try:
            ...
            if pujo is None:
                ...
                logger.error(
                    f"Error: {response_data['error']}", extra={"user_id": user_id}
                )
    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            transport_df = pd.DataFrame.from_records(
                Metro.objects.all().values("id", "lat", "lon")
            )

            if not transport_df.empty:
                nearest_transport_id, nearest_distance = find_nearest_transport(
                    transport_df, target_coords
                )
            else:
            ...
            serializer.save(
                metro_id=nearest_transport_id, nearest_metro_distance=nearest_distance
            )
            # serializer.save()
            response_data = {
            ...
            user_id = request.user.id if request.user.is_authenticated else None
            logger.info(
                f"Success: {response_data['message']}", extra={"user_id": user_id}
            )
            ...
            user_id = request.user.id if request.user.is_authenticated else None
            logger.error(
                f"Error: {str(response_data['error'])}", extra={"user_id": user_id}
            )
    ...
    def destroy(self, request, uuid=None, *args, **kwargs):
        try:
            ...
            user_id = request.user.id if request.user.is_authenticated else None
            logger.info(
                f"Success: {response_data['result']}", extra={"user_id": user_id}
            )
            return Response(response_data, status=status.HTTP_200_OK)
        except Pujo.DoesNotExist:
            response_data = {
    ...
    def increase_search_score(self, request, *args, **kwargs):
        try:
            ...
            if serializer.is_valid(raise_exception=True):
                ...
                missing_ids = [
                    str(pujo_id) for pujo_id in ids if str(pujo_id) not in found_pujos
                ]
                ...
                if term == "search":
                    for pujo_id, pujo in found_pujos.items():
                        ...
                        pujo.save()
                        log.append(
                            {"id": str(pujo_id), "result": "Score decremented by 1"}
                        )
                ...
                if term == "select":
                    ...
                    for pujo_id, pujo in found_pujos.items():
                        ...
                        pujo.save()
                        log.append(
                            {"id": str(pujo_id), "result": "Score incremented by 2"}
                        )
                    ...
                    for pujo_id, pujo in found_pujos.items():
                        ...
                        pujo.save()
                        log.append(
                            {"id": str(pujo_id), "result": "Score incremented by 3"}
                        )
    ...
    def search_pujo(self, request, *args, **kwargs):
        try:
            ...
            if serializer.is_valid():
                ...
                return Response(
                    {"error": serializer.errors, "status": ResponseStatus.FAIL.value},
                    status=status.HTTP_400_BAD_REQUEST,
                )
